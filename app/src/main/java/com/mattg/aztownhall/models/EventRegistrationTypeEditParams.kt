...
/**
 *
 * @param Id Unique event identifier. This field should be provided only when updating existing event registration type
 * @param EventId Identifier of event related to the registration type.
 * @param Name Registraton type title. This field should be provided only when creating new registration type
 ...
 * @param BasePrice Base price registrant pays with this registration type.
 * @param GuestPrice Price for each additional guest.
 * @param UseTaxScopeSettings Indicates whether account tax scope settings should be used to calculate final price. ⚠ Once set, it has to be provided on every update. Otherwise, it will be overwritten to default value
 * @param Availability
 ...
 * @param AvailableFrom This registration type is available starting from this date. Can be null. ⚠ Once set, it has to be provided on every update. Otherwise, it will be overwritten to null
 * @param AvailableThrough This registration type is available until this date. Can be null. ⚠ Once set, it has to be provided on every update. Otherwise, it will be overwritten to null
 ...
 * @param CancellationDaysBeforeEvent If CancellationBehaviour = AllowUpToPeriodBeforeEvent, this field contains minimum number of days before the event that cancellations are allowed
 * @param IsWaitlistEnabled Indicated whether registrant should be placed on waitlist if registrations have reached their limit.
 */
data class EventRegistrationTypeEditParams(
